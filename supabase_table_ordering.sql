-- =============================================================
-- PLAY ROOM ARTICHOKE — comenzi la masă prin sesiuni QR
...
CREATE OR REPLACE FUNCTION public.admin_get_tables()
RETURNS TABLE (
...
AS $$
  ...
  GROUP BY tables.table_number, sessions.id, sessions.token, sessions.opened_at
  ORDER BY tables.table_number;
$$;

CREATE OR REPLACE FUNCTION public.admin_open_table_session(p_table_number INTEGER)
RETURNS TABLE (session_id UUID, token UUID, opened_at TIMESTAMPTZ)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
...
BEGIN
  ...
  RETURN QUERY SELECT v_session.id, v_session.token, v_session.opened_at;
END;
$$;
